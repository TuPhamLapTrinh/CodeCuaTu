try:
    ...
    def hello(name):
        ...
        if day_time < 12:
            ...
            speak("Chào buổi chiều bạn {}. Bạn đã dự định gì cho chiều nay chưa.".format(name)) # @@!11
        else:
    ...
    def send_email(text):
        ...
        if 'gửi' in recipient:
            ...
            mail.ehlo()
            mail.starttls()
            mail.login('tuitdes@gmail.com', 'Tuitdes123')
            mail.sendmail('tuitdes@gmail.com',
                          'phamtucc234@gmail.com', content.encode('utf-8'))
            mail.close()
    ...
    def current_weather():
        ...
        ow_url = "https://openweathermap.org/weathermap?basemap=map&cities=true&layer=temperature&lat=20.2274&lon=106.6058&zoom=10"
        city = get_text()
        if not city:
            pass
    ...
    def tell_me_about():
        try:
            speak("Bạn muốn nghe về gì ạ") # đã fix
            text = get_text()
    ...
    def assistant():
        #speak("Xin chào, bạn Tú A2K50 Hoa Lư A")
        #name = get_text()
        name = "Tú IT "
        #name1="Bạn cần Bot Alex có thể giúp gì ạ?"
        if name:
            speak("Chào bạn {}".format(name))
           # speak("Hmm{}".format(name1))
